...
inline void SerializeType(std::ostream& stream, const void* data, TypeId type)
{
	...
	else
	{
		...
		auto& members = MemberId::GetMemberInfos(type);
...
inline void DeserializeType(std::istream& stream, void* data, TypeId id)
{
	...
	else
	{
		...
		auto& members = MemberId::GetMemberInfos(id);
		while (buffer != '}')
		{
			Deserialize(stream, MemberName);

			auto member = std::find_if(members.begin(), members.end(), [&MemberName](const MemberId::MemberInfo& info) {return info.Name == MemberName; });
